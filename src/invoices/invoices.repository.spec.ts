...
describe('InvoicesRepository', () => {
  ...
  it('creates invoice', async () => {
    ...
    expect(prismaMock.client.invoice.create).toHaveBeenCalledWith({
      data: dto,
    });
  ...
  it('finds by id', async () => {
    ...
    expect(prismaMock.client.invoice.findUnique).toHaveBeenCalledWith({
      where: { invoiceId: '1' },
    });
  ...
  it('updates invoice', async () => {
    ...
    expect(prismaMock.client.invoice.update).toHaveBeenCalledWith({
      where: { invoiceId: '1' },
      data: { total: 6 },
    });
