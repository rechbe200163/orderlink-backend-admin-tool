...
describe('AddressesRepository', () => {
  ...
  it('creates address', async () => {
    ...
    expect(prismaMock.client.address.create).toHaveBeenCalledWith({
      data: dto,
    });
  ...
  it('finds all addresses', async () => {
    ...
    expect(prismaMock.client.address.findMany).toHaveBeenCalledWith({
      where: { deleted: false },
    });
  ...
  it('finds address by id', async () => {
    ...
    expect(prismaMock.client.address.findUnique).toHaveBeenCalledWith({
      where: { addressId: '1' },
    });
  ...
  it('updates address', async () => {
    prismaMock.client.address.findUnique.mockResolvedValue({
      id: 1,
      city: 'A',
    });
    prismaMock.client.address.update.mockResolvedValue({ id: 1, city: 'B' });
    const result = await repository.update('1', { city: 'B' } as any);
    expect(prismaMock.client.address.update).toHaveBeenCalledWith({
      where: { addressId: '1' },
      data: { city: 'B' },
    });
